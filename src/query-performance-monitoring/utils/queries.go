	...
	*/
	MariaDBBlockingSessionsQuery = `
		...
		SELECT
			...
			wt.PROCESSLIST_HOST AS blocked_host,
			wt.PROCESSLIST_DB AS database_name,
			COALESCE(wt.PROCESSLIST_STATE, 'Lock Wait') AS blocked_status,
			b.trx_id AS blocking_txn_id,
			b.trx_mysql_thread_id AS blocking_thread_id,
			...
			COALESCE(wt.DIGEST, IF(r.trx_query IS NULL, NULL, MD5(r.trx_query))) AS blocked_query_id,
			COALESCE(bt.DIGEST, IF(b.trx_query IS NULL, NULL, MD5(b.trx_query))) AS blocking_query_id,
			COALESCE(bt.PROCESSLIST_STATE, 'Idle in transaction') AS blocking_status,
			ROUND(COALESCE(TIMESTAMPDIFF(MICROSECOND, r.trx_wait_started, NOW()), 0) / 1000, 3) AS blocked_query_time_ms,
